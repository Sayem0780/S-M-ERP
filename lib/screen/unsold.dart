...
class _UnsoldScreenState extends State<UnsoldScreen>
  ...
  void searchCar(String cNo) {
    String chassisNO = cNo.toLowerCase().trim(); // Convert the search text to lowercase for case-insensitive comparison

    List<Chassis> matchingLC = chassisList.where((element) => element.name.toLowerCase().contains(chassisNO)).toList();
    setState(() {
      if (matchingLC.isNotEmpty) {
        chassisList = matchingLC;
        hideSecondSegment();
      }else if(chassisNO.length==0){
        chassisList = itemList;
      }else{
        chassisList = itemList;
      }
