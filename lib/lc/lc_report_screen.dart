...
class _LcReportScreenState extends State<LcReportScreen> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    // dynamic checker = ModalRoute.of(context)!.settings.arguments as dynamic;
    // checker!=null?tapLCIndex= int.parse(checker.toString()):null;
    // checker!=null?isSecondSegmentVisible = true:null;
    // checker!=null?selectedListTileIndex= int.parse(checker.toString()):null;
    return WillPopScope(
      onWillPop: ()async{
        box.close();
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.done) {
              if (snapshot.hasData) {
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            ...
                            child: Padding(
                              padding: isSecondSegmentVisible? EdgeInsets.all(8):EdgeInsets.symmetric(horizontal: 400,vertical: 20),
                              child: ListTile(
                                shape: RoundedRectangleBorder(
                                  borderRadius: BorderRadius.circular(10), // Set your desired border radius
                                  side: BorderSide(width: 2, color: Colors.black), // Set your desired border side
                                ),
                                trailing: IconButton(
                                  onPressed: () {
                                    showDialog(
                                      ...
                                      builder: (context) {
                                        return AlertDialog(
                                          ...
                                          content: Text('Are you sure you want to delete this LC?'),
                                          actions: [
                                            TextButton(
                                              child: Text('Cancel'),
                                              onPressed: () {
                          ...
                          child: Column(
                            ...
                            children: [
                              Center(child: Padding(
                                padding: const EdgeInsets.all(8.0),
                                child: Text('LC NO: ${selectedLcMap[tapLCIndex]?.lcNo??""}',style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),),
                              )),
                              Row(
                ...
                return Center(child: Text('No LC data available.'));
              }
            } else {
